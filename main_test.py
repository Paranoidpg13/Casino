...
class IntroPage(tk.Frame):
    def __init__(self, parent, main_view, *args, **kwargs):
        ...
        # Canvas for marquee lights
        self.canvas = tk.Canvas(self, width=800, height=100, bg="black", highlightthickness=0)
        self.canvas.pack(pady=20)

        self.lights = []
        self.light_colors = ["red", "yellow", "green", "blue", "purple"]
        self.num_lights = 24
        self.create_lights()
        ...
        # Title label (fade-in)
        self.title_label = tk.Label(self, text=" Welcome to MachaPuchre Casino", font=("Helvetica", 28, "bold"),
                                    fg="white", bg="black")
        ...
        # Start button
        self.start_button = tk.Button(self, text="Enter Casino", font=("Helvetica", 16), bg="darkgreen", fg="red",
                                      activebackground="green", command=self.start_game)
    ...
    def create_lights(self):
        ...
        for i in range(self.num_lights):
            x = i * spacing + 20
            light = self.canvas.create_oval(x, 30, x+20, 50, fill="gray", outline="")
            self.lights.append(light)

    def animate_lights(self):
        for i, light in enumerate(self.lights):
            color = self.light_colors[(self.current_color_index + i) % len(self.light_colors)]
            self.canvas.itemconfig(light, fill=color)
        self.current_color_index = (self.current_color_index + 1) % len(self.light_colors)
        self.after(150, self.animate_lights)

    def fade_in_title(self):
        if self.fade_level <= 255:
            hex_color = f"#{self.fade_level:02x}{self.fade_level:02x}{self.fade_level:02x}"
            self.title_label.config(fg=hex_color)
            self.fade_level += 5
            self.after(50, self.fade_in_title)
        else:
            self.title_label.config(fg="white")

    def start_game(self):
        self.main_view.bj.show()
